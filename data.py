...
def load_voc():
    ...
    if (not (os.path.exists(DEFINES.vocabulary_path))):
        ...
        words = [word for sentance in data for word in tokenizing_data(sentance)]
        # 중복되는 단어(토큰)를 제거
...
def make_voc(voc_list):
    ...
    return dict(zip(voc_list, range(len(voc_list)))), dict(zip(range(len(voc_list)), voc_list))
